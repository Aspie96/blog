...
The idea I suggest is, in essence, that of making the training process artificially biased in the same way that we are biased, so that we can communicate a task to it with just a few examples. Because the training of *big network* has observed a distribution of tasks that we care about, inference on *big network*, which is also training of *small network*, can extract that information and gravitate towards implicit task descriptions that are actually interesting.
